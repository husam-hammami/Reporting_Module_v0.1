...
| **Fix: order table time +4h vs DB** | API may append **`Z`** to naive `start_time`/`end_time`. **`formatDateTime`** now strips trailing **`Z`** before `new Date()` so the table matches PostgreSQL wall time (same idea as historian `from`/`to`). |
