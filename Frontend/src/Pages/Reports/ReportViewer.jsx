...
function SingleReportView({ reportId, onBack }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] bg-transparent">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-center">
            {TIME_PRESETS.map((p) => (
              <button
                ...
                onClick={() => {
                  setTimePreset(p.id);
                  if (p.id === 'custom') setShowCustomPanel(true);
      ...
      {/* ── Custom date range (only when custom selected and panel is open) ── */}
      {timePreset === 'custom' && showCustomPanel && (
        <div className="bg-white/90 dark:bg-[#0a1525] backdrop-blur-sm border-b border-[#e3e9f0] dark:border-gray-700 px-4 py-2.5 flex items-center gap-3 flex-wrap print:hidden">
          {/* Shift quick-select buttons */}
          {shiftsConfig?.shifts?.length > 0 && (
            <div className="flex items-center gap-2 mr-2">
              <span className="text-[11px] font-medium text-[#6b7f94]">Shift:</span>
              {shiftsConfig.shifts.map((s, i) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    setCustomTo(toLocal(to));
                  }}
                  className="px-2.5 py-1 text-[11px] font-medium rounded-md border border-[#e3e9f0] dark:border-gray-700 text-[#3a4a5c] dark:text-[#c1ccd9] hover:border-brand hover:bg-brand-subtle transition-colors"
                >
                  {s.name}
                </button>
              ))}
              <div className="w-px h-5 bg-[#e3e9f0] dark:bg-gray-700 mx-1" />
            </div>
          )}
          ...
          <button
            ...
            className="px-4 py-1.5 text-[11px] font-semibold rounded-md bg-brand text-white hover:bg-brand-hover transition-colors ml-1"
          >
            Apply
          </button>
        </div>
