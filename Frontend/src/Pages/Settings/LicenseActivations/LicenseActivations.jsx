...
import { FaKey, FaCheck, FaTimes, FaTrash, FaSync, FaCalendarPlus, FaEdit, FaDesktop, FaInfoCircle } from 'react-icons/fa';
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-[#e3e9f0] dark:border-[#1e2d40]">
                  <th className="text-left py-2 px-3 text-[10px] font-semibold uppercase text-[#6b7f94]">Label / Hostname</th>
                  <th className="text-left py-2 px-3 text-[10px] font-semibold uppercase text-[#6b7f94]">Machine ID</th>
                ...
                {licenses.map(lic => (
                  <React.Fragment key={lic.id}>
                    <tr className="border-b border-[#e3e9f0] dark:border-[#1e2d40] hover:bg-[#f5f8fb] dark:hover:bg-[#0d1825]">
                      ...
                      <td className="py-2.5 px-3">
                        {editLabelId === lic.id ? (
                          ...
                          <div className="flex items-center gap-1.5">
                            <div>
                              {lic.label ? (
                                <div className="font-medium text-[#2a3545] dark:text-[#e1e8f0]">{lic.label}</div>
                              ) : null}
                              <div className={`text-[11px] ${lic.label ? 'text-[#8898aa]' : 'text-[#2a3545] dark:text-[#e1e8f0]'}`}>
                                {lic.hostname || '—'}
                              </div>
                            </div>
                            <button
                              ...
                              title="Edit label"
                              className="text-[#8898aa] hover:text-brand transition-colors p-0.5"
                            >
                              <FaEdit size={9} />
                            </button>
                          </div>
                      ...
                      <td className="py-2.5 px-3 font-mono text-[10px] text-[#8898aa]" title={lic.machine_id}>
                      ...
                      {/* Expiry */}
                      <td className="py-2.5 px-3">
                        <span className={isExpired(lic.expiry, lic.status) ? 'text-red-500 font-medium' : 'text-[#2a3545] dark:text-[#e1e8f0]'}>
                          {formatDate(lic.expiry)}
                        </span>
                        {isExpired(lic.expiry, lic.status) && (
                      ...
                      <td className="py-2.5 px-3">
                        <button
                          ...
                          className={`p-1 rounded transition-colors ${expandedId === lic.id ? 'text-brand bg-brand/10' : 'text-[#8898aa] hover:text-brand'}`}
                        >
                          <FaDesktop size={11} />
                        </button>
                      </td>
                    ...
                    {expandedId === lic.id && (
                      <tr className="bg-[#f8fafc] dark:bg-[#0a1220]">
                        <td colSpan={7} className="px-6 py-3">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-[11px]">
                            <InfoField label="MAC Address" value={lic.mac_address} />
                            <InfoField label="IP Address" value={lic.ip_address} />
                            <InfoField label="OS" value={lic.os_version} />
                            <InfoField label="CPU" value={lic.cpu_info} />
                            <InfoField label="RAM" value={lic.ram_gb ? `${lic.ram_gb} GB` : null} />
                            <InfoField label="Disk Serial" value={lic.disk_serial} />
                            <InfoField label="User ID" value={lic.user_id} />
                            <InfoField label="Created" value={formatDate(lic.created_at)} />
                          </div>
                          <div className="mt-2 font-mono text-[10px] text-[#8898aa] break-all">
                            Full ID: {lic.machine_id}
                          </div>
                        </td>
                      </tr>
                    )}
...
function InfoField({ label, value }) {
  return (
    <div>
      <div className="text-[9px] uppercase tracking-wider text-[#8898aa] mb-0.5">{label}</div>
      <div className="text-[#2a3545] dark:text-[#e1e8f0]">{value || '—'}</div>
    </div>
  );
}
