...
# ── POST /mappings/seed ────────────────────────────────────────────────────
@mappings_bp.route('/mappings/seed', methods=['POST'])
def seed_mappings():
    ...
    try:
        ...
        with closing(conn) as conn:
            ...
            cursor.execute("SELECT COUNT(*) as cnt FROM mappings")
            count = cursor.fetchone()['cnt']
            if count > 0:
                return jsonify({
                    'status': 'success',
                    'message': f'Mappings table already has {count} entries, skipping seed',
                    'seeded': 0
                })
            ...
            for m in SEED_MAPPINGS:
                ...
                """, (m['name'], m.get('description'), m['input_tag'], m['output_tag_name'],
                      json.dumps(m['lookup']), m.get('fallback', 'Unknown')))
                seeded += 1

            conn.commit()
            logger.info(f"Seeded {seeded} default mappings")
            return jsonify({
                'status': 'success',
                'message': f'Seeded {seeded} default mappings',
                'seeded': seeded
            }), 201

    except Exception as e:
        logger.error(f"Error seeding mappings: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
